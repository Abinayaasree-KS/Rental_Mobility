...
class car
{
    ...
    void change_status(char* reg_num,car *tptr)
    {
        ...
        myfile.open("Car Details.DAT",ios::binary|ios::in);
    ...
    bool return_check()
    {
        ...
        if(myfile.is_open())
        {
            ...
            {
                cout<<"\n\t\tNone of the cars are need to be returned!!";
                return false;
            }
        }
        else
    ...
    void return_status(char* reg_num,char* car_name)
    {
        ...
        if(myfile.is_open())
        {
            ...
            remove("Car Details.DAT");
            rename("Temp.DAT","Car Details.DAT");
            if(check==1&&front!=NULL)
            {
                this->supply_queue(reg_num,car_name);
            }
            else if(check==1&&front==NULL)
            {
                cout<<"\n\tCar has been successfully returned";
            }
            else if(check==0)
            cout<<"Yes,problem is here !!";
        }
        else
        {
            cout<<"\n\tError !!";
    ...
    void supply_queue(char *reg_num,char *car_name)
    {
        ...
        temp.open("Temporary.DAT",ios::binary|ios::out|ios::app);
        if(myfile.is_open())
        {
    ...
    void display_cll(car *ptr)
    {
        ...
        if(min_vacant==1)
        {
            while(check==1)
            {
                ...
                cout<<"\n\t\t(Next Car = 1 / Book this car = 2 / Exit = 3)";
                cin.ignore();
                cin>>check;
                if(check==2)
                {
...
void create_pass()
{
    ...
    cin>>create.password;
    fstream myfile;
    myfile.open("Secured.DAT",ios::binary|ios::out);
    myfile.write((char*)&create,sizeof(struct login));
    myfile.close();
    seek=1;
}
void change_pass()
{
    ...
    cin>>create.password;
    fstream myfile;
    myfile.open("Secured.DAT",ios::binary|ios::in|ios::out);
    if(myfile.is_open())
    {
        myfile.read((char*)&x,sizeof(struct login));
        if(strcmp(x.username,create.username)==0&&strcmp(x.password,create.password)==0)
        {
            cout<<"\n\tEnter the new password :";
            cin>>x.password;
            myfile.seekp(0,ios::beg);
            myfile.write((char*)&x,sizeof(struct login));
    ...
    cout<<"\n\tPassword File Doesn't Exists..";
}
void check_pass()
{
    fstream myfile;
    myfile.open("Secured.DAT",ios::binary|ios::in);
    if(myfile.is_open())
    {
        if(!myfile.eof())
            seek=1;
